...
class WithdrawalLoop {
  ...
  final HandlePosition? customerHandlePositionSide; // Which side of customer the handle is on
  ...
  const WithdrawalLoop({
    ...
    this.hoursValue,
    this.customerHandlePositionSide,
  ...
  factory WithdrawalLoop.createWithHandles({
    ...
    ConnectionHandle? supermarketHandle, // Optional supermarket handle for proper approach direction
    ConnectionHandle? customerHandle, // Optional customer handle for proper departure direction
  }) {
    // If we have both handles, calculate optimal kanban position along the efficient path
    Offset kanbanIconPosition;
    List<Offset> pathPoints;
    
    if (supermarketHandle != null && customerHandle != null) {
      ...
      pathPoints = calculatePath(customerHandlePosition, supermarketHandlePosition, kanbanIconPosition, supermarketHandle, customerHandle);
    } else {
      // Fallback to original logic
      kanbanIconPosition = calculateInitialKanbanPosition(customerHandlePosition, supermarketHandlePosition);
      pathPoints = calculatePath(customerHandlePosition, supermarketHandlePosition, kanbanIconPosition, supermarketHandle, customerHandle);
    }
    ...
    return WithdrawalLoop(
      ...
      kanbanIconPosition: kanbanIconPosition,
      hoursValue: null, // Initialize with null
      customerHandlePositionSide: customerHandle?.position,
  ...
  WithdrawalLoop updateKanbanPosition(Offset newKanbanPosition) {
    ...
    if (customerHandlePositionSide != null) {
      startHandle = ConnectionHandle(
        itemId: customerProcessId,
        itemType: 'process', // Default type for processes
        position: customerHandlePositionSide!,
        alignment: HandleAlignment.center, // Default alignment
        offset: Offset.zero, // Not needed for path calculation
      );
    }
  ...
  WithdrawalLoop copyWith({
    ...
    Offset? kanbanIconPosition,
    double? hoursValue,
    HandlePosition? customerHandlePositionSide,
    ...
    return WithdrawalLoop(
      ...
      customerHandlePositionSide: customerHandlePositionSide ?? this.customerHandlePositionSide,
